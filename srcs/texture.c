...
/*   Updated: 2021/03/26 20:27:15 by tblink           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(tab->n_tex->img = mlx_xpm_file_to_image(tab->mlx_p,
		tab->params->north, &(tab->n_tex->width), &(tab->n_tex->height))))
	{
	...
	if (!(tab->s_tex->img = mlx_xpm_file_to_image(tab->mlx_p,
		tab->params->south, &(tab->s_tex->width), &(tab->s_tex->height))))
	{
...
static void		add_texture_2(t_tab *tab)
{
	...
	if (!(tab->sprite->img = mlx_xpm_file_to_image(tab->mlx_p,
		tab->params->sprite, &(tab->sprite->width), &(tab->sprite->height))))
	{
